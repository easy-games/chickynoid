...
function module:WritePlayerPositions(serverTime)
	...
	snapshot.players = {}
	for key,playerRecord in pairs(players) do
	...
	for counter = #self.history,1,-1 do
		local snapshot = self.history[counter]
		
		--only keep 1s of history
		if (snapshot.serverTime < serverTime-1) then
...
function module:PushPlayerPositionsToTime(playerRecord, serverTime)
	...
	if (debugFlag == true) then
		print("Prev time ", prevRecord.serverTime,  " Next Time ", nextRecord.serverTime,  " des time ", serverTime, " frac ", frac)
	end
	...
	for userId, prevRecord in pairs(prevRecord.players) do
		
		if (userId == playerRecord.userId) then
			continue	--Dont move us
		end
		
		local nextRecord = nextRecord.players[userId] 
		...
		if (otherPlayerRecord.chickynoid.hitBox) then
			...
			local pos = prevRecord.position:Lerp(nextRecord.position, frac)
