...
function Simulation:RegisterMoveState(
    name,
    updateState,
    alwaysThink,
    startState,
...
function Simulation:MovetypeWalking(cmd)
    ...
    if wishDir ~= nil then
        if onGround then
            ...
            flatVel =
                MathUtils:Accelerate(wishDir, self.constants.airSpeed, self.constants.airAccel, flatVel, cmd.deltaTime)
        end
    else
        if onGround ~= nil then
            ...
            flatVel = MathUtils:VelocityFriction(flatVel, self.constants.airBrake, cmd.deltaTime)
        end
    end
        ...
        if cmd.y > 0 then
            if self.state.jumpThrust > 0 then
                ...
                self.state.jumpThrust =
                    MathUtils:Friction(self.state.jumpThrust, self.constants.jumpThrustDecay, cmd.deltaTime)
            end
