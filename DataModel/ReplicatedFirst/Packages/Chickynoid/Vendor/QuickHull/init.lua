...
local vertex = require(script:WaitForChild("vertex", 1))
local face = require(script:WaitForChild("face", 1))
...
function quick_hull.new(points)
	...
	self.vertices = {}
	for i = 1, #points do
		table.insert(self.vertices, vertex.new(points[i], i))
	end
	self.discardedFaces = {}
...
function quick_hull:addAdjoiningFace(eyeVertex, horizonEdge)
	local face = face.createTriangle(
		eyeVertex,
		horizonEdge:tail(),
		horizonEdge:head()
	)
...
function quick_hull:addVertexToHull(eyeVertex)
	...
	for i = 1, #self.newFaces do
		...
		if (face.mark == VISIBLE) then
			while (self:doAdjacentMerge(face, MERGE_NON_CONVEX_WRT_LARGER_FACE)) do end
		end
	end
	...
	for i = 1, #self.newFaces do
		...
		if (face.mark == NON_CONVEX) then
			face.mark = VISIBLE
			while (self:doAdjacentMerge(face, MERGE_NON_CONVEX)) do end
		end
	end
